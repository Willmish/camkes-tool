...
/*- if buffer is none -*/
  /*? establish_from_rpc(connector, language='rust') ?*/
/*- else -*/
  ...
  /*- else -*/
    /*- set lock = False -*/
  /*- endif -*/
  /*? establish_from_rpc(connector, buffer=('((void*)%s)' % c[0].from_end.interface.name, macros.dataport_size(c[0].from_end.interface.type), lock), language='rust') ?*/
/*- endif -*/
