...
def thread_stack(sym, size, language='c'):
    if language == 'rust':
...
#[link_section = "align_%(page_size_bits)sbit"]
#[no_mangle]
static mut %(sym)s: %(sym)s = %(sym)s { data: [0u8; %(page_size)s * 3] };
''' % {"sym": sym, "page_size": PAGE_SIZE, "page_size_bits": 12}
    elif language == 'c':
...
def ipc_buffer_address(sym, language='c'):
    if language == 'rust':
        return '%s.data[%s]' % (sym, PAGE_SIZE)
    elif language == 'c':
...
def shared_buffer_symbol(sym, shmem_size, page_size, language='c'):
    page_size_bits = int(math.log(page_size, 2))
    if language == 'rust':
        return '''
#[allow(non_upper_case_globals)]
...
#[link_section = "align_%(page_size_bits)sbit"]
#[no_mangle]
pub static mut %(sym)s: %(sym)s = %(sym)s { data: [0u8; %(shmem_size)s] };
...
def show_type(t, language='c'):
    def show_string_type_c(t):
        if t == 'string':
            return 'char *'
        elif t == 'character':
            return 'char'
    ...
    if isinstance(t, six.string_types):
        if language == 'c':
            return show_string_type_c(t)
        elif language == 'rust':
            return show_string_type_rust(t)
        else:
            raise Exception('Unsupported language %s' % language)
    else:
...
def type_to_fit_integer(value, language='c'):
    def type_to_fit_c(value):
        if value <= 2 ** 8:
            return 'uint8_t'
        elif value <= 2 ** 16:
            return 'uint16_t'
    ...
    if language == 'c':
        return type_to_fit_c(value)
    elif language == 'rust':
        return type_to_fit_rust(value)
    else:
        raise Exception('Unsupported language %s' % language)
...
def show_attribute_value(t, value, language='c'):
...
def from_interface_index(composition, instance, interface):
    assert isinstance(composition, Composition)
    assert isinstance(instance, Instance)

    for connection in composition.connections:
        for index, f in enumerate(connection.from_ends):
            if f.instance == instance and f.interface == interface:
                return index
    print ('NO MATCH', instance, interface)
    return 0


def has_to_end(composition, instance, interface):
    assert isinstance(composition, Composition)

    for connection in composition.connections:
        for index, t in enumerate(connection.to_ends):
            if t.instance == instance and t.interface == interface:
                return True
    print ('NO TO ', instance, interface)
    return False


def include_code(composition, instance, interface):
    assert isinstance(composition, Composition)
    assert isinstance(instance, Instance)

    for c in composition.connections:
        for index, f in enumerate(c.from_ends):
            if f.instance == instance and f.interface == interface:
                return '%s_%s_%s' % (interface, c.type.name, index)
        for index, t in enumerate(c.to_ends):
            if t.instance == instance and t.interface == interface:
                return '%s_%s_%s' % (interface, c.type.name, index)

    return None  # NB: an unconnected "maybe" object


def interface_is_irq(interface, instance):
    assert isinstance(instance, Instance)

    return interface in instance.type.consumes


def dataport_size(type, language='c'):
    assert isinstance(type, six.string_types)
    m = re.match(r'Buf\((\d+)\)$', type)
    if m is not None:
        return m.group(1)
    if language == 'rust':
        return 'core::mem::size_of::<%s>()' % show_type(type, language)
    else:
        return 'sizeof(%s)' % show_type(type, language)


def dataport_type(type, language='c'):
    assert isinstance(type, six.string_types)
    if re.match(r'Buf\((\d+)\)$', type) is not None:
        return 'void'
    return show_type(type, language)
